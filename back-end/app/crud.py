...
def add_feed_item(db: Session, feed_item: dict):
    ...
    existing_item = db.query(FeedItem).filter_by(title=title, pubDate=pubDate, link=link).first()
    if existing_item:
        return
...
def update_president_schedule(db: Session, president_schedule: dict):
    time, location, description = president_schedule['time'], president_schedule['location'], president_schedule['description']
